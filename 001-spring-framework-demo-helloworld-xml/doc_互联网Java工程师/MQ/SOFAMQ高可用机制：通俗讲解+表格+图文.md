...
1. Queue 是实际存在于 Broker（快递中转站）的 “消息存储单元”（相当于中转站的具体仓库）；
2. 创建 Topic A 时，会生成你指定数量的 Queue，这些 Queue 会被 “标记归属 Topic A”（相当于给仓库贴 “Topic A 专属” 标签）；
3. 这些 “Topic A→对应 Queue→所在 Broker” 的映射关系，会同步到 NameServer（快递调度中心），供查询；
4. 生产者发消息给 Topic A 时，先问 NameServer “Topic A 的 Queue 在哪个 Broker 上”，拿到地址后，就把消息发到这些 “标记了 Topic A” 的 Queue 里。
